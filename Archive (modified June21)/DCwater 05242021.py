...
fig, ax = plt.subplots()
ax.plot('time', 'inf_flo', data = waterDComb[waterDComb['cluster'] == 0])
...
fig, ax = plt.subplots()
ax.plot('time', 'inf_flo', data=waterDComb[waterDComb['cluster'] == 1])
six_month_interval = mdates.MonthLocator(interval=6)
ax.xaxis.set_major_locator(six_month_interval) ###defines major axis every 6 months
month = mdates.MonthLocator()
ax.xaxis.set_minor_locator(month) ###defines minor axis every month
...
data0 = waterDComb[waterDComb['cluster'] == 0].reset_index()
...
data1 = waterDComb[waterDComb['cluster'] == 1].reset_index()
...
#     for i in range(0, len(waterDComb)):
#         if flowdata[i] >= flowMax and flowdata[i+1] >= flowdata[i] and flowdata[i] >= flowdata[i+2]:
#             stormTime = time[i+1]
#             print('stormDay', stormTime)            
#stormDay = storm_date(waterDComb.FI_PLTINF)

def storm_date(flowdata):
    for i in range(0, len(data0)):
        if flowdata[i] >= flowMax and flowdata[i+1] >= flowdata[i] and (flowdata[i] <= flowdata[i+2] or flowdata[i] >= flowdata[i+2]):
            stormTime = time[i+1] 
            print('stormDay', stormTime)
...
time = data1['time']
flowMax = np.percentile(data1['inf_flo'], 95)
stormTime = []
...
stormDay  = storm_date(data1.inf_flo)
