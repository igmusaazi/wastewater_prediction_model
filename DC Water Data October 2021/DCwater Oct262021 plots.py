...
######plotting data################
fig, ax = plt.subplots()
ax.plot('datetime', 'ammonia_ppm_imp', color = 'darkred',linestyle = 'solid',label ='ammonia (ppm) ', data = storm)
ax.plot('datetime', 'nitrate_ppm_imp',color = 'black', linestyle = 'solid', label = 'nitrate(ppm)', data = storm)

ax.set_ylabel('effluent concentration (ppm)', fontweight = 'bold')
ax.ticklabel_format(axis='y', style='sci', scilimits=(0,0), useOffset=None, useLocale=None, useMathText=True)
ax.tick_params(color = 'black', labelcolor='black',labelsize = 'small', width=2)
plt.legend(loc = 2)
ax1 = ax.twinx()
ax1.plot('datetime', 'complete_flow_imp',color = 'darkgreen', linestyle = 'solid', label = 'effluent flow (MGD)', data = storm)
ax1.set_ylabel('effluent flow (MGD)', fontweight = 'bold') 
...
plt.title('Effluent Quality Changes (April 2018)', fontweight = 'bold',fontsize = 11)
hour_six = mdates.MinuteLocator(interval=600)  #mark every six hours
hour = mdates.HourLocator()
ax.xaxis.set_minor_locator(hour) ###defines minor axis every minute
ax.xaxis.set_major_locator(hour_six) ###defines minor axis every minute

ax.xaxis.set_major_formatter(mdates.DateFormatter('%m-%d(%I:%M %p)')) 
ax.set_xlabel('Month-Day(Time)', fontweight = 'bold',fontsize = 12)
fig.autofmt_xdate() # Rotates and right aligns the x labels
plt.savefig('BluePlains Flow Quality and April Rainfall.png',dpi = 1200, bbox_inches='tight')


storm = pd.read_csv('../DC Water Data October 2021/dcWaterStorms(October 2021)/july2019.csv')
...
fig, ax = plt.subplots()
ax.plot('datetime', 'performance', color = 'blue',linestyle = 'solid',label ='flow relative difference (%) ', data = storm)
ax.set_ylabel('flow relative difference (%)', fontweight = 'bold')
ax.ticklabel_format(axis='y', style='sci', scilimits=(0,0), useOffset=None, useLocale=None, useMathText=True)
ax.tick_params(color = 'black', labelcolor='black',labelsize = 'small', width=2)
plt.legend(loc = 7)
ax1 = ax.twinx()
ax1.plot('datetime', 'prcp_hr',color = 'black', linestyle = 'solid', label = 'rainfall intensity (inches/hr)', data = storm)
ax1.set_ylabel('rainfall intensity (inches/hr)', fontweight = 'bold') 
ax1.ticklabel_format(axis='y', style='sci', scilimits=(0,0), useOffset=None, useLocale=None, useMathText=True)
ax1.tick_params(axis = 'y', labelcolor='k', labelsize='medium', width=2)
plt.legend(loc = 1)

plt.title('July 2019 storm', fontweight = 'bold',fontsize = 11)
hour_six = mdates.MinuteLocator(interval=600)  #mark every six hours
hour = mdates.HourLocator()
ax.xaxis.set_minor_locator(hour) ###defines minor axis every minute
ax.xaxis.set_major_locator(hour_six) ###defines minor axis every minute
...
ax.set_xlabel('Month-Day(Time)', fontweight = 'bold',fontsize = 12)
fig.autofmt_xdate() # Rotates and right aligns the x labels
plt.savefig('BluePlains Flow and July Rainfall.png',dpi = 1200, bbox_inches='tight')


storm = pd.read_excel('../DC Water Data October 2021/dcWaterStorms(October 2021)/resiliency results.xlsx',sheet_name = Sheet2)
storm['datetime']= pd.to_datetime(storm['datetime'])
timeDay = pd.Series(range(len(storm['datetime'])), index=storm['datetime'])/24
storm['time'] = timeDay.reset_index(drop = True)
storm['performance'] = (1 - ((storm['complete_flow_imp']-storm['influent_flow_imp'])/(storm['influent_flow_imp'])))*100
...
fig, ax = plt.subplots()
ax.plot('rainfall intensity (inches/hr) ', 'recovery time (hr)NH4 ', color = 'darkred',linestyle = 'solid',label ='ammonia (ppm) ', data = storm)
ax.plot('rainfall intensity (inches/hr) ', 'recovery time (hr)NO3-'  , color = 'black', linestyle = 'solid', label = 'nitrate(ppm)', data = storm)
...
plt.legend(loc = 1)
ax1 = ax.twinx()
ax1.plot('datetime', 'time equivalent performance reduction(ppm*hr)NH4',color = 'gold', linestyle = 'solid', label = 'effluent flow (MGD)', data = storm)
ax1.plot('datetime', 'time equivalent performance reduction(ppm*hr) NO3-',color = 'gold', linestyle = 'solid', label = 'effluent flow (MGD)', data = storm)
...
plt.title('Effluent Quality Changes (October 2019 storm)', fontweight = 'bold',fontsize = 11)
one_month_interval = mdates.MonthLocator(interval=1)
...
fig.autofmt_xdate() # Rotates and right aligns the x labels
plt.savefig('BluePlains Flow Quality and Oct Rainfall.png',dpi = 1200)
...
#storm = pd.read_csv('../DC Water Data October 2021/dcWaterStorms(October 2021)/resiliency correlation.csv', header=1)
#storm.corr(method='spearman')

storm = pd.read_csv('../DC Water Data October 2021/dcWaterStorms(October 2021)/october2019.csv')
storm['datetime']= pd.to_datetime(storm['datetime'])
timeDay = pd.Series(range(len(storm['datetime'])), index=storm['datetime'])/24
storm['time'] = timeDay.reset_index(drop = True)
storm['performance'] = (1 - ((storm['complete_flow_imp']-storm['influent_flow_imp'])/(storm['influent_flow_imp'])))*100
...
recovery_time(storm.nitrate_ppm_imp)
...
storm = pd.read_csv('../dcWaterStorms(October 2021)/december2018.csv')
stormDRY = storm[storm['prcp_hr']==0].reset_index(drop=True)
stormWET = storm[storm['prcp_hr']>0].reset_index(drop=True)
scipy.stats.ttest_ind(stormDRY.ammonia_ppm_imp, stormWET.ammonia_ppm_imp, equal_var=False)
stormDRY.ammonia_ppm_imp.describe()


#scipy.stats.spearmanr(storm, b=None)